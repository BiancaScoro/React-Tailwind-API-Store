...
const Sidebar = () => {
  ...
  return (
    ...
    w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      ...
      <div className='flex flex-col gap-y-2 h-[520px] lg:h-[640px] overflow-y-auto oveflow-x-hidden border-b'>{cart.map((item) => {
        return <CartItem item={item} key={item.id} />
      })}
      </div>
      <div className='flex flex-col gap-y-3 py-4'>
        <div className='bg-pink-500 flex w-full justify-between items-center'>
